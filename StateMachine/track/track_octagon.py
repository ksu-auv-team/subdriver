...
class Track_Octagon(Sub):
    ...
    def execute(self, userdata):
        ...
        while(1):
            ...
            #TODO: Change get_boxes_of_classes to only return the highest confidence detection of each class
            # detections = self.get_boxes_of_classes(gbl.detections, gbl.current_target)
            ...
            if detections != None:
                ...
                if (oct_found):
                    oct_center = self.getCenter(oct_det.box)
                    if (self.get_distance(oct_det.box) > 0.8):
                        self.is_close = True
                if (coffin_found):
                    coffin_center = self.getCenter(coffin_det.box)
                    if (self.get_distance(coffin_det.box) > 0.8):
                        self.is_close = True
                if (oct_found and coffin_found):
                    ...
                    center_of_both = self.getCenter[min(oct_center.x, coffin_center.x), \
                    max(oct_center.y, coffin_center.y), max(oct_center.x, coffin_center.x), \
                    min(oct_center.y, coffin_center.y)]
